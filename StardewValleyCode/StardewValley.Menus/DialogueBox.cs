	...
	public class DialogueBox : IClickableMenu
	{
		...
		public DialogueBox(string dialogue, List<Response> responses, int width = 1200)
		{
			...
			if (responses != null && Game1.options.SnappyMenus)
			{
				...
				int num = this.y - (this.heightForQuestions - this.height) + SpriteText.getHeightOfString(this.getCurrentString(), width - Game1.pixelZoom * 4) + Game1.pixelZoom * 12;
				for (int i = 0; i < responses.Count; i++)
				{
					this.responseCC.Add(new ClickableComponent(new Rectangle(this.x + Game1.pixelZoom * 2, num, width - Game1.pixelZoom * 2, SpriteText.getHeightOfString(responses[i].responseText, width - Game1.pixelZoom * 4) + Game1.pixelZoom * 4), "")
					{
						myID = i,
						downNeighborID = ((i < responses.Count - 1) ? (i + 1) : -1),
						upNeighborID = ((i > 0) ? (i - 1) : -1)
					});
					num += SpriteText.getHeightOfString(responses[i].responseText, width) + Game1.pixelZoom * 4;
				}
		...
		{
			if ((!Game1.options.SnappyMenus || !this.isQuestion) && (Game1.options.doesInputListContain(Game1.options.actionButton, key) || Game1.options.doesInputListContain(Game1.options.menuButton, key)) && Game1.options.SnappyMenus && !Game1.options.doesInputListContain(Game1.options.actionButton, key))
			{
		...
		private void setUpQuestions()
		{
			...
			if (this.responses != null && Game1.options.SnappyMenus)
			{
				...
				for (int i = 0; i < this.responses.Count; i++)
				{
					this.responseCC.Add(new ClickableComponent(new Rectangle(this.x + Game1.pixelZoom * 2, num, this.width - Game1.pixelZoom * 2, SpriteText.getHeightOfString(this.responses[i].responseText, this.width - Game1.pixelZoom * 4) + Game1.pixelZoom * 4), "")
					{
						myID = i,
						downNeighborID = ((i < this.responses.Count - 1) ? (i + 1) : -1),
						upNeighborID = ((i > 0) ? (i - 1) : -1)
					});
		...
		public override void update(GameTime time)
		{
			...
			if (this.transitioning)
			{
				...
				{
					bool arg_26A_0 = this.transitionWidth != 0;
					...
					if (!arg_26A_0 && this.transitionWidth > 0)
					{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (this.transitioning)
			{
				...
				if ((!this.activatedByGamePad || Game1.lastCursorMotionWasMouse || this.isQuestion || Game1.isGamePadThumbstickInMotion(0.2)) && (Game1.getMouseX() != 0 || Game1.getMouseY() != 0) && (!Game1.options.SnappyMenus || this.isQuestion))
				{
					base.drawMouse(b);
				}
				return;
